...
class SaleView(SubmenuView):
    ...
    def __init__(self, buy_or_sell=None, timeout=None):
        ...
        if buy_or_sell not in ["buy", "sell"]:
            logger.warning(f"Invalid buy_or_sell value: {buy_or_sell}")
            return
    ...
    async def select_category(self, interaction: discord.Interaction):
        ...
        await interaction.message.edit(f"## Jason's Shop\nBrowsing {self.selected_category}s...", view=self)
    ...
    async def select_item_callback(self, interaction: discord.Interaction):
        ...
        self.qty_confirm.callback = self.on_qty_confirm_clicked
        self.qty_cancel.callback = self.on_qty_cancel_clicked
    ...
    async def update_quantity(self, interaction: discord.Interaction, amount: int):
        verb = "Buying" if self.buy_or_sell == "buy" else "Selling"
    ...
    async def on_qty_confirm_clicked(self, interaction: discord.Interaction):
        ...
        if self.on_purchase_callback:
            ...
            from cogs.shop import create_receipt
            from models.users import UserModel

            success = await UserModel.give_item(
                interaction.user.id,
                self.selected_item,
                self.quantity,
                self.cost_total
            ...
            if success:
                ...
                await interaction.message.edit(content="Transaction completed.", view=None, embed=receipt)
            else:
                await interaction.response.send_message("You don't have enough to buy that.", ephemeral=True)

    async def on_qty_cancel_clicked(self, interaction: discord.Interaction):
        self.disable_all_items()
        await interaction.message.edit("Transaction cancelled.", view=self)

    def update_cost_total(self):
        if self.buy_or_sell == "buy":
            self.cost_total = self.full_selected_item.cost * self.quantity
        else:
            self.cost_total = -1 * self.full_selected_item.resell_price * self.quantity
